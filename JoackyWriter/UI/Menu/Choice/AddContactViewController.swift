...
class AddContactViewController: UIViewController, UITextFieldDelegate {
    ...
    @IBOutlet weak var lblVwTitle: UILabel!
   ...
   // var isUserName : Bool = false
    ...
    override func viewDidLoad() {
        ...
        lblName.textColor = UIColor.darkBlue.withAlphaComponent(isEdit ? 1.0 : 0.5)
        lblNumber.textColor = UIColor.darkBlue.withAlphaComponent(isEdit ? 1.0 : 0.5)
       // btnAdd.isEnabled = isEdit ? true : false
       ...
       // tfTitle.delegate = self
        ...
        lblVwTitle.text = "ADD CONTACT"
        if isEdit {
            lblVwTitle.text = "EDIT CONTACT"
            btnAdd.setTitle("Done", for: .normal)
            let model = arrContact[selectedIndex!]
            lblName.text = model.name
            lblNumber.text = model.contactNumber
            imgVw.image = APPDELEGATE.loadImageFromDocumentDirectory(nameOfImage: model.imgName!)
            strUserName = model.name
            strPhoneNumber = model.contactNumber
            tempPhoneNumber = model.contactNumber!
            
            checkForImage(image: imgVw.image!)
    ...
    deinit {
        ...
        debugPrint("AddContactViewController deinit")
    }
...
//        self.navigationController?.show(imagePicker, sender: self)
    ...
    func checkForImage(image: UIImage) {
    ...
    func openActionSheetForImageCapture(){
        ...
        DispatchQueue.main.async {
            ...
            CameraHandler.shared.imagePickedBlock = { image, imgURL in
                
             //   if image.size.width > 200.0 && !Utils.isPad {
                    
                    self.checkForImage(image: image)
//                    if let compressed = image.resized(toWidth: 200.0) {
//                        self.imgVw.image = compressed
//                        if self.strUserName != nil && self.strPhoneNumber != nil {
//                            self.btnAdd.isEnabled = true
...
//                    self.imgVw.image = image
    ...
    func addContact(){
        ...
        if arrContact.contains(where: {$0.contactNumber == strPhoneNumber}) {
            ...
            if let index = arrContact.firstIndex(where: {$0.contactNumber == strPhoneNumber}), let sIndex = selectedIndex {
    ...
    func saveOrUpdateContact() {
        if APPDELEGATE.saveImageToDocumentDirectory(image: imgVw.image!, imageName: strPhoneNumber!) {
            ...
            let id = isEdit ? arrContact[selectedIndex!].id : "\(arrContact.count + 1)"
            let arrDict : [String: String] = ["id" : "\(id!)" ,
                                                 "name" : strUserName!,
            ...
            do {
                ...
                if isEdit {
                    arrContact[selectedIndex!] = tempDict
                } else {
                    ...
                    if arr.count > 0 {
                        ...
                        if isEdit {
                            ...
                            self.dismiss(animated: true) { [weak self] in
                                if self!.tempPhoneNumber != self!.strPhoneNumber && self!.isEdit {
                                   
                                       
                                    APPDELEGATE.removeItemForDocument(itemName: self!.tempPhoneNumber, fileExtension: "png")
    ...
    @objc func enterName() {
        self.strUserName = isEdit ? arrContact[selectedIndex!].name : ""
    ...
    @objc func enterPhoneNumber() {
        self.strPhoneNumber = isEdit ? arrContact[selectedIndex!].contactNumber : ""
