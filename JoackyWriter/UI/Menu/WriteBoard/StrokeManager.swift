...
class StrokeManager {
  ...
  */
  ...
  private var inkModel: DigitalInkRecognitionModel?
    ...
    NotificationCenter.default.addObserver(
      ...
      using: { [weak self]
        ...
        if notification.userInfo![ModelDownloadUserInfoKey.remoteModel.rawValue]
            as? DigitalInkRecognitionModel == weakSelf.inkModel
        {
         // weakSelf.delegate?.displayMessage(message: "Model download succeeded")
            
            DispatchQueue.main.async {
                APPDELEGATE.hideLoadingView()
                Toast.show(message: "Model has been downloaded successfully.", controller: (APPDELEGATE.window?.rootViewController)!)
               // APPDELEGATE.window?.rootViewController?.showToast(message: "Please wait. Require file is being downloaded.", font: UIFont.systemFont(ofSize: 17))
            }
        }
      })
    NotificationCenter.default.addObserver(
      ...
      using: {
        ...
        (notification) in
        if notification.userInfo![ModelDownloadUserInfoKey.remoteModel.rawValue]
          as? DigitalInkRecognitionModel == self.inkModel
        {
            DispatchQueue.main.async {
                Toast.show(message: "Model download failed.", controller: (APPDELEGATE.window?.rootViewController)!)
               // APPDELEGATE.window?.rootViewController?.showToast(message: "Please wait. Require file is being downloaded.", font: UIFont.systemFont(ofSize: 17))
            }
            //APPDELEGATE.window?.rootViewController?.showToast(message: "Model has been downloaded successfully.", font: UIFont.systemFont(ofSize: 17))
   ...
   */
  func downloadModel() {
    if modelManager.isModelDownloaded(inkModel!) {
     // self.delegate?.displayMessage(message: "Model is already downloaded")
      return
    }
    DispatchQueue.main.async {
        APPDELEGATE.showLoadingView()
        Toast.show(message: "Please wait. Require file is being downloaded.", controller: (APPDELEGATE.window?.rootViewController)!)
       // APPDELEGATE.window?.rootViewController?.showToast(message: "Please wait. Require file is being downloaded.", font: UIFont.systemFont(ofSize: 17))
    }
  ...
  func recognizeInk() {
    ...
    if recognizer == nil {
      ...
      let options: DigitalInkRecognizerOptions = DigitalInkRecognizerOptions.init(model: inkModel!)
