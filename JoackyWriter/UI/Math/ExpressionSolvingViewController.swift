//
//  ExpressionSolvingViewController.swift
//  JoackyWriter
//
...
class ExpressionSolvingViewController: UIViewController, DrawingViewDelegate, UITextFieldDelegate{
    ...
    private var padFixedFontSize = 250.0 as CGFloat
    ...
    override func viewDidLoad() {
        ...
        if Account.writerOption == 0 {
            ...
            typeTextfield.keyboardType = UIKeyboardType.numbersAndPunctuation
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        Sounds.default.speak(text.replacingOccurrences(of: "-", with: " minus "))
        setupDrawingViews()
...
//                let nLetter = self.text.characters[self.text.index(self.text.startIndex, offsetBy: 0)]
//
//                Sounds.default.speak(String(nLetter))
            }
        ...
        if Utils.isLandscapeMode == true || UIScreen.main.bounds.height == 480.0 {
            ...
            if scrollView.isHidden {
                ...
                if Utils.isPad {
                    ...
                    {
                        typeLblHeight.constant = 430.0
                        
                        typeLblScrollViewHeight.constant = 430.0
                        
                        padFixedFontSize = 250.0
                    }
    ...
    func setupDrawingViews() {
        ...
        for i in 0..<count {
            ...
            drawingView.frame = CGRect.init(x: currentPos,
                                            ...
                                            height: scrollView.frame.size.height)
            if i == 0 || i == 2 || i == 4 {
                var width = letterWidth
                var numberString = String.init(letter)
                if i == 4 && i != count - 1 {
                    let nextLetter = text[text.index(text.startIndex, offsetBy: i + 1)]
                    numberString.append(nextLetter)
                    width += DrawingView.width(for: nextLetter)
                }
                let height: CGFloat = Utils.isPad ? 135.0 : 90.0
                let frame = CGRect.init(x: currentPos,
                                        y: scrollView.frame.size.height - height,
                                        width: width,
                                        height: height)
                let circesCount = Int.init(numberString)
                let circlesView = CirclesView.init(frame: frame, circlesCount: circesCount!)
                contentView.addSubview(circlesView)
            }
            currentPos += letterWidth
            drawingView.backgroundColor = UIColor.clear
            //            drawingView.backgroundColor = i % 2 == 0 ? UIColor.red : UIColor.yellow
...
//                        Sounds.default.speak(String(nLetter))
    ...
    func drawingViewDidCompleteDraw(drawingView: DrawingView) {
        ...
        if currentDrawingViewIndex == string.count - 1 {
            attemptIndex += 1
            tokenEconomyBoardView.score = attemptIndex
            Sounds.default.playSuccessSound()
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                ...
                let index = string.index(string.startIndex, offsetBy: self.currentDrawingViewIndex)
...
//                    }
                ...
                if (String(letter) == " ") {
                    ...
                    Sounds.default.speak(String(letter))
